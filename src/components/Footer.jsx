import { Link } from 'react-router-dom'
import './Footer.css'

const quickLinks = [
  { label: 'About Us', path: '/about' },
  { label: 'Products', path: '/products' },
  { label: 'Geography', path: '/geography' },
  { label: 'Contact', path: '/contact' },
]

const products = [
  { label: 'Nitrogen Fertilizers', anchor: 'nitrogen' },
  { label: 'Phosphate Fertilizers', anchor: 'phosphate' },
  { label: 'Potash & Complex', anchor: 'potash' },
  { label: 'Agricultural Products', anchor: 'agricultural' },
  { label: 'Petrochemicals', anchor: 'petrochemical' },
]

function Footer() {
  return (
    <footer className="footer">
      <div className="footer__separator" />
      <div className="footer__main">
        <div className="container">
        <div className="footer__grid">
          {/* Company */}
          <div className="footer__col footer__col--about">
            <Link to="/" className="footer__logo">
              <div className="footer__logo-icon">S</div>
              <div className="footer__logo-text">
                <span className="footer__logo-name">SILKWAY</span>
                <span className="footer__logo-sub">COMMODITIES</span>
              </div>
            </Link>
            <p className="footer__about-text">
              International commodity trading company specializing in fertilizers,
              agricultural products, and petrochemicals.
            </p>
          </div>

          {/* Quick Links */}
          <div className="footer__col">
            <h4 className="footer__col-title">Company</h4>
            <ul className="footer__links">
              {quickLinks.map((link) => (
                <li key={link.path}>
                  <Link to={link.path}>{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Products */}
          <div className="footer__col">
            <h4 className="footer__col-title">Key Products</h4>
            <ul className="footer__links">
              {products.map((p) => (
                <li key={p.anchor}>
                  <Link to={`/products#${p.anchor}`}>{p.label}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div className="footer__col">
            <h4 className="footer__col-title">Get in Touch</h4>
            <ul className="footer__contact">
              <li>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0l-9.75 6-9.75-6"/>
                </svg>
                <a href="mailto:trade@silkwaycommodities.com">trade@silkwaycommodities.com</a>
              </li>
              <li>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"/>
                </svg>
                <a href="tel:+998505402235">+998 50-540-22-35</a>
              </li>
              <li>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/>
                  <path d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/>
                </svg>
                <span>Dubai, UAE · Tashkent, UZ</span>
              </li>
            </ul>
          </div>
        </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="footer__bottom">
        <div className="container footer__bottom-inner">
          <p>&copy; {new Date().getFullYear()} Silkway Commodities. All rights reserved.</p>
          <div className="footer__bottom-links">
            <a href="#">Privacy Policy</a>
            <a href="#">Terms of Service</a>
          </div>
        </div>
      </div>
    </footer>
  )
}

export default Footer
